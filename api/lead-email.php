...
if (strpos($type, 'application/json') !== false) { $raw=file_get_contents('php://input'); $input=json_decode($raw ?: '', true); if (!is_array($input)) out(400,['ok'=>false,'error'=>'Некорректный JSON']); } else { $input=$_POST; }
...
$digits=preg_replace('/\D+/', '', $phone); if (strlen((string)$digits)<10 || strlen((string)$digits)>15) out(400,['ok'=>false,'error'=>'Проверьте телефон']);
